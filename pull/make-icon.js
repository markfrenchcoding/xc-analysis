...
//
// Writes apple-touch-icon.png and og.png at the repo root.
//
...
// the title, which is where the bare "C" came from. It wants a real PNG.
//
// The same drawing also makes the card that shows up when the link is shared.
// That card carries the mark alone and no wordmark, because this encoder has no
// font: drawing letters would mean shipping a rasteriser to say something the
// link preview already says in its title.
//
// With the page mark and the tab icon that is four copies of one formation, and
// none of them can share a source - the page bakes its scale into the
// coordinates, the tab icon is a data URI that cannot read a custom property,
// and these two are pixels. Redraw the seven and all four need it.
//
...
const JOBS = [
  // fill: how much of the SHORT side the formation takes. iOS masks the icon
  // square to a squircle and crops a few percent off every edge, so the mark
  // sits well inside it. The share card is letterboxed and wants more air.
  { out: path.join(__dirname, '..', 'apple-touch-icon.png'), w: 180, h: 180, fill: 0.74 },
  { out: path.join(__dirname, '..', 'og.png'), w: 1200, h: 630, fill: 0.46 },
];
...
for (const d of FILLED.concat(HOLLOW)) {
...
function render({ out, w, h, fill }) {
  const W = w * SS, H = h * SS;
  const scale = (Math.min(W, H) * fill) / Math.max(x1 - x0, y1 - y0);
  ...
  for (const d of HOLLOW) {
    const [cx, cy] = T(d.c), r = d.r * scale, sw = d.w * scale;
  ...
  console.log('wrote ' + path.basename(out) + '  ' + w + 'x' + h
